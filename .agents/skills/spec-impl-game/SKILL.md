---
name: spec-impl-game
description: Implements an approved spec (identical flow to /spec-impl), then automatically runs the skin-designer and mobile-porter audit agents in sequence — skin-designer first, mobile-porter second, never in parallel. Intended for specs that add or change a game in the catalog.
disable-model-invocation: true
argument-hint: <NN-spec-name>
allowed-tools: Bash(git status:*), Bash(git branch:*), Bash(git checkout:*), Bash(cat:*), Bash(ls:*), Agent
---

# /spec-impl-game — Implementer of approved game specs + audit chain
...
Follow these five phases in strict order. **Do not advance to the next phase if the previous one did not complete correctly.** Phases 1–4 are identical to `/spec-impl` — only Phase 5 is new.
...
**When finishing the last step of the plan**, do not print the `/spec-impl` closing block yet — continue straight into Phase 5. The plan being fully implemented is the trigger condition for Phase 5, not a stopping point.

---

### Phase 5 — Post-implementation audits (skin-designer → mobile-porter)

This phase only starts once every step in the spec's implementation plan is implemented and confirmed by the user in Phase 4. Do not enter Phase 5 early, and do not skip it — it is the reason this command exists instead of plain `/spec-impl`.
...
1. Tell the user:
   ...
   Now running the two catalog audit agents, one after another (never in parallel):
     1. skin-designer — makes sure the game(s) touched by this spec have their 3 skins.
     2. mobile-porter — makes sure any new/changed pages or the player shell stay responsive.

   Starting with skin-designer.
   ```

2. Invoke the `Agent` tool with `subagent_type: "skin-designer"`. Give it a short, self-contained prompt naming the spec just implemented (e.g. `Recién se implementó specs/NN-slug.md en la rama spec-NN-slug. Audita el catálogo de skins y cubre lo que falte.`) — the agent orients itself from its own memory and `JUEGOS.md`, it does not need more than that.

3. **Do not invoke mobile-porter until skin-designer's completion notification has arrived.** This is the hard "sequential, not parallel" requirement: one `Agent` call, wait for its result in a later turn, only then the second `Agent` call. Never put both `Agent` calls in the same message.

4. Once skin-designer's result arrives, invoke the `Agent` tool with `subagent_type: "mobile-porter"`, same pattern: short prompt naming the spec/branch just implemented, and mention that skin-designer already ran in this same chain (so it knows fresh CSS/markup may already differ from its memory).

5. Wait for mobile-porter's completion notification.

6. Print a single closing summary combining all three stages:

   ```
   ✅ Spec implemented + audited — specs/NN-slug.md

   Implementation: all plan steps done on branch spec-NN-slug.
   skin-designer:  <one-line verdict from its own closing report>
   mobile-porter:  <one-line verdict from its own closing report>
   ...
   this includes whatever skin-designer/mobile-porter left in the working tree.
   ```

Do not paraphrase away the two agents' findings — if either reported `parcial` or `pendiente` for something, surface that in the one-line verdict instead of flattening it to a generic "done".
...
```
/spec-impl-game 07-nuevo-juego

  Phase 1  →  Finds specs/07-nuevo-juego.md
  Phase 2  →  Reads the state → "Approved" (or "Aprobado", etc.) → ✅ continues
  Phase 3  →  git checkout -b spec-07-nuevo-juego → git checkout spec-07-nuevo-juego
              Shows objective, scope, plan and criteria
  Phase 4  →  Implements step by step with pauses
              Last step confirmed → falls through to Phase 5 (no closing block yet)
  Phase 5  →  Agent(subagent_type: "skin-designer") → wait for its notification
              Agent(subagent_type: "mobile-porter") → wait for its notification
              Prints combined summary + acceptance-criteria/merge reminder

/spec-impl-game 08-otro-juego  (state: Draft / Borrador)

  Phase 1  →  Finds specs/08-otro-juego.md
...
**The audit chain in Phase 5 always runs skin-designer before mobile-porter, and only after the entire plan is implemented** — never per-step, never in parallel, never before Phase 4 is fully confirmed done.
