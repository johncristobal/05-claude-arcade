...
- `mobile-porter` (`.claude/agents/mobile-porter.md`) — audits and fixes **responsive/mobile layout** for marketing pages and the player shell (`.crt`/`.crt-screen`/`.player-hud`). Invoke explicitly ("usa mobile-porter"). Like `skin-designer`, it **does write code**: `Read, Grep, Glob, Write, Edit, Bash` (the last only for `npm run lint`). It never touches touch-input territory already closed by `specs/11-controles-tactiles.md` (`lib/games/touchInput.ts`, `components/game/TouchControls.tsx`, per-game `*_TOUCH_CONFIG`), `lib/supabase/**`, `components/AuthProvider.tsx`, or existing specs. There's no PWA/manifest/native-wrapper scaffolding in the repo, and adding one is out of scope — it's responsive web only. It reads its own memory → `specs/11-controles-tactiles.md` (precedent) → `app/globals.css` → `app/layout.tsx` → `components/Nav.tsx` → `app/juegos/[id]/jugar/page.tsx` → marketing pages → `CLAUDE.md`/`AGENTS.md`, then adjusts/adds media queries in `app/globals.css` and decides on an `export const viewport` in `app/layout.tsx` (gated on checking Next 16's default viewport emission first). It closes each run by writing a `specs/NN-mobile-responsive.md` (`Estado: Approved` — already implemented) and updating its own memory `.claude/agents/mobile-porter/memoria.md` (`Fecha | Página/sección | Breakpoints tocados | Estado | Notas`, states `cumple` · `parcial` · `pendiente`).

Chain: **game-planner decides** (catalog-gap-driven) or **game-jam generates** (theme-driven) → for game-jam, the user promotes a chosen Draft variant to a numbered spec → `/add-game` writes the spec (game-planner path) or the promoted draft is used as-is (game-jam path) → `/spec-impl` implements it. `skin-designer` and `mobile-porter` run orthogonally to this chain — neither adds games; one retrofits skins, the other retrofits responsive layout onto whatever is already in the catalog.
